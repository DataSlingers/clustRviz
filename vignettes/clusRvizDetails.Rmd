...
Convex clustering is traditionally solved by iteratively applying ADMM updates until convergence.  This must be done for each $\lambda_k$ which is computationally expensive.

To address the computational burden of Convex Clustering,
we utilize the framework of Algorithmic Regularization Paths 
...
Rather than fully solving the Convex Clustering optimization problem via 
traditional iterative optimization techniques, we instead 
...
Once fit, both the clustering solution of both CARP and CBASS may be examined 
...
along the regularization path; we plot the new cluster labels along with 
the original weight graph.
...
`Clustering` function. In the example below we extract the biclustering solution
