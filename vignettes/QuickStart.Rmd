...
In the sections below we work through an example dataset, 
...
for now we show how `clustRviz` can quickly be used to achieved 
...
Additionally, since our variable are measured on different scales, we 
...
In the plot above displays the first two principal components of the 
...
join together, observations are fused together and clusters are formed.
...
# display the labels
head(carp.clusters$clustering.assignment)
...
```{r}
cbass.fit <- CBASS(Xdat)
```

Once fitted, vizualizations are displayed again via the `plot` function. 
For CBASS fits, the default visualization is the bicluster heatmap:
